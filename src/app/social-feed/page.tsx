...
export default function SocialFeedPage() {
  ...
  const profileEntries = entries.filter((e): e is Extract<SocialFeedEntry, { type: "profile" }> => e.type === "profile");

  const platformDescription = (platform: string) => {
    const descs = t.platformDescriptions as Record<string, string>;
      ...
      <section className="relative z-10 w-full max-w-md mx-auto px-4 pt-12 pb-8">
        ...
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-500">{t.socialFeedSubtitle}</p>
          </div>
        </div>
        ...
        {loading ? (
            ...
            {Array.from({ length: 3 }).map((_, i) => (
              <Card key={i} className="w-full border-0 shadow-lg shadow-slate-200/50 dark:shadow-black/30 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl animate-pulse">
                <CardContent className="p-4">
                  ...
                  <div className="w-3/4 h-3 rounded bg-slate-200 dark:bg-slate-700 mb-2" />
          ...
          <div className="space-y-6">
            ...
            {youtubePosts.length > 0 && (
              <div>
                <div className="flex items-center gap-2 mb-3">
                  <div className="flex items-center justify-center w-7 h-7 rounded-lg bg-[#FF0000]/10 dark:bg-[#FF0000]/20">
                    <span className="text-[#FF0000]"><PlatformIcon platform="youtube" className="w-3.5 h-3.5" /></span>
                  </div>
                  <h2 className="text-sm font-bold text-slate-900 dark:text-white">{t.latestVideos}</h2>
                </div>
            ...
            <div>
              <div className="flex items-center gap-2 mb-3">
                <div className="flex items-center justify-center w-7 h-7 rounded-lg bg-orange-500/10 dark:bg-orange-500/20">
                  <svg className="w-3.5 h-3.5 text-orange-600 dark:text-orange-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
                </div>
                <h2 className="text-sm font-bold text-slate-900 dark:text-white">{t.myProfiles}</h2>
              </div>
              <div className="space-y-2">
                {profileEntries.map((entry) => {
                  if (entry.type !== "profile") return null;
                  return (
                    <a
                      key={entry.platform}
                      href={entry.url}
                      ...
                      <Card className="w-full border-0 shadow-lg shadow-slate-200/50 dark:shadow-black/30 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl transition-transform duration-300 hover:scale-[1.02] cursor-pointer">
                        <CardContent className="p-4">
                          <div className="flex items-center gap-3">
                            <div className={`flex items-center justify-center w-10 h-10 rounded-xl ${platformBg[entry.platform]}`}>
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs text-slate-400 dark:text-slate-500 truncate">{entry.username}</p>
                              {platformDescription(entry.platform) && (
                                <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">{platformDescription(entry.platform)}</p>
                              )}
                            </div>
                            <div className="flex items-center gap-1.5 text-orange-600 dark:text-orange-400">
                              <span className="text-[11px] font-medium">{t.visitProfile}</span>
